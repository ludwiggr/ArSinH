...
\begin{document}
    ...
    Unabhängig welche Reihe genutzt wird, können die Zwischenergebnisse $x^{\pm k}$ entweder für sehr große Werte oder sehr kleine Werte von $x$ nicht als $double$ dargestellt werden und entwerten damit das Endergebnis.
    Daher haben Reihendarstellungen stets begrenzten Gültigkeitsbereiche.
    ...
    In der Vergleichsimplementierung ohne die Beschränkung auf eine reine Reihendarstellung, kann das $double$ Datenformat genutzt werden, um die Berechnung signifikant auf Genauigkeit zu optimieren. Die Implementierung mit verschiedenen Reihen verwendet eine Bitmaske um effizient Exponent und Mantisse von $x$ ermitteln.
