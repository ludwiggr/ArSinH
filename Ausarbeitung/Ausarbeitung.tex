...
\begin{document}
    ...
    Eine \textbf{Reihenentwicklung mit mehreren Reihen} trifft hier eine einzige Fallunterscheidung für $x\leq1$, um die Anzahl an Fallunterscheidungen gering zu halten.
    ...
    %TODO: Absatz evtl Löschen ?
    Für die Reihendarstellungen und die Lookup-Tabelle wird die Punktsymmetrie der $arsinh$-Funktion: $arsinh(-x) = -arsinh(x)$ genutzt.
    Die Funktionswerte für negative Eingabewerte werden per Fallunterscheidung mit $-arsinh(|x|)$ berechnet.


    \subsection{Reihendarstellung}\label{subsec:reihendarstellung}
    ...
    Unter anderem wird die Methode der kleinsten Quadrate verwendet, um geeignet Koeffizienten für Reihenglieder mit positiven oder negativen Potenzen zu finden.
    Die auf diese Weise hergeleitete Reihenentwicklung liefert allerdings für Werte zwischen den verwendeten Datenpunkten zu ungenaue Ergebnisse, da die $arsinh$-Funktion nicht linear ist.

    Zusätzlich wurden Reihenentwicklungen für verschiedene Intervalle hergeleitet, die im Folgenden näher erläutert werden sollen.
    ...
    \]
    Um diese beiden Reihen zu einer reinen Reihe zu vereinigen, gilt die folgende Umformung:
    \[
    ...
    In der Vergleichsimplementierung ohne die Beschränkung auf eine reine Reihendarstellung, kann das $double$ Datenformat genutzt werden, um die Berechnung signifikant auf Genauigkeit zu optimieren.
    Die Implementierung mit verschiedenen Reihen verwendet Exponent und Mantisse von $x$ um folgende Umformung zu treffen:
    \begin{gather*}
    ...
    Durch Anwendung des Horner-Schemas lässt sich die Reihe besonders effizient berechnen.

    \subsubsection{Umsetzung}
    In der Implementierung mit einer reinen Reihenentwicklung wird die \textit{TaylorArsinh} verwendet.
    ...
    \]
    Aus dieser Formel lässt sich bereits ablesen, dass ein Reihenglied kleiner ist, je kleiner x ist. %TODO: Ein Reihengleid? nicht das reihengleid, auf das sich oben bezogen wird?
    ...
    Für die gemischte Reihe lassen sich ähnliche Berechnungen mit \textit{TaylorLn} und \textit{Restreihe} aufstellen.
    Um ein sinnvolles Verhältnis von Genauigkeit und Performanz zu erhalten, wird Anzahl der zu berechnenden Reihenglieder für \textit{TaylorArsinh} und \textit{Restreihe} ebenfalls auf 13 festgelegt.
    Bei \textit{TaylorLn} werden 27 Reihenglieder berechnet, um mit minimaler Anzahl der Glieder auf dem gesamten Wertebereich ein exaktes Ergebnis zu erreichen.

    %TODO: nicht hier!
    Es sei an dieser Stelle bereits erwähnt, dass ausgehend von diesen Untersuchungen eine weitere Optimierung umsetzbar wäre, die (beispielsweise mithilfe des Exponenten des Eingabewertes) die Anzahl der nötigen Reihenglieder bestimmt und diese berechnet.
    ...
    \begin{figure}[h]
        \begin{tabular}{@{}c@{}}
        \includegraphics[width = 12cm,height = 6cm]{images/Figure3}
        \smash Abbildung 1
        \end{tabular}
        
        \begin{tabular}{@{}c@{}}
        \includegraphics[width = 7cm,height = 6cm]{images/Figure1}
        \includegraphics[width = 7cm, height = 6cm]{images/Figure2}
        \small Abbildung 2
        \end{tabular}
        \label{fig:Abbildung 1-3}
    \end{figure}
    ...
    %TODO: Abbildungen nicht so benannt, wie im Text referneziert
    ...
    %TODO: Abbildung nicht so benannt
    Wie in Abb.1 zu sehen ist, liefert die Implementierung mit komplexen mathematischen Instruktionen der C-Math Bibliothek für den Großteil des Wertebereichs den genauest möglichen Funktionswert für den Datentyp double.
    ...
    Der folgende Graph zeigt die durchschnittliche Laufzeit eines Funktionsaufrufs der drei Implementierungen mit Reihenentwicklung, Tabellen-Lookup und der Implementierung mithilfe komplexer Instruktionen der C-math-library in Nanosekunden:
     ...
     \begin{tikzpicture}
         \begin{axis}[
             ...
             ybar,
             enlargelimits = 0.12,
             symbolic x coords = {gemischte Reihe, reine Reihe,  Lookup-Tabelle,  komplex},
             xtick={gemischte Reihe, reine Reihe, Lookup-Tabelle, komplex},
             ylabel={Zeit in ns},
             ...
             \addplot coordinates {(gemischte Reihe, 6.569) (reine Reihe,6.369) (Lookup-Tabelle, 2.613) (komplex, 1.648)};
             \addplot coordinates {(gemischte Reihe, 29.212) (reine Reihe, 6.369) (Lookup-Tabelle, 2.613) (komplex, 1.648)};
             \addplot coordinates {(gemischte Reihe, 29.212) (reine Reihe,6.369) (Lookup-Tabelle, 2.613) (komplex, 0.0)};
    ...
    %TODO: Abbildungen konkret referenzieren, nicht in den abbildungen
    ...
    Diese und gegebenenfalls weitere Optimierungen könnten die Implementierung des $arsinh(x)$ noch weiter verbessern, bleiben hier aus Gründen der Übersichtlichkeit sowie der gegebenen Beschränkungen allerdings unberücksichtigt.
