...
\begin{document}
    ...
    Reihendarstellungen verwenden in der Regel Polynome zur Berechnung eines Näherungswertes. Die einzelnen Potenzen der Reihenglieder können sowohl positiv, als auch negativ sein. Reihenglieder mit positive Potenzen werden für große Eingabewerte $|x|>>1$, Reihenglieder mit negativen Potenzen für Eingabewerte $|x|<<1$ zu einem Double Overflow führen.  Es ist aus diesem Grund allein aus Darstellungsgründen nicht möglich, eine reine Reihenentwicklung zu finden, die für alle als double darstellbaren Eingabewerte eine sinnvolle Näherung liefert und nicht bei bestimmten Eingaben zu $\pm \inf$ evaluiert. Auch die Entwicklung einer Laurent-Reihe, die in der Regel den gesamten Definitionsbereich einer Funktion annähert, ist nicht möglich, da die arsinh-Funktion keine Singularitäten aufweist. 
    ...
    Sowohl für diese Implementierung, als auch für die Lookuptabelle haben wir uns die Punktsymmetrie der $arsinh$-Funktion zunutze gemacht: $arsinh(-x) = -arsinh(x)$. Die Funktionswerte für negative Eingabewerte, werden als $-arsinh(|x|)$ berechnet.
