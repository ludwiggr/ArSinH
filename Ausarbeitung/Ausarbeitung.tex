 ...
 \begin{document}
     ...
     \section{Einleitung}
     In den Bereichen der Control-Theory und Datenverarbeitung gibt es zahlreiche Anwendungen, in denen die Berechnung bestimmter mathematischer Funktionen von großer Bedeutung ist. Eine dieser Funktionen ist der \textit{Areasinus Hyperbolicus} ($arsinh(x)$), der unter anderem die Beschreibung nichtlinearer Beziehungen und Modellierung von Sättigungsprozessen ermöglicht. Das liegt an der Eigenschaft des $arsinh(x)$ für größer werdende Inputwerte immer langsamer zu steigen, aber ohne eine Asymptote zu besitzen. Um die Genauigkeit und Effizienz solcher Berechnungen zu garantieren, ist es von entscheidender Bedeutung, die Berechnung des $arsinh(x)$ effizient umzusetzen.  
     ...
     Unsere Aufgabe ist hierbei eben diesen $arsinh(x)$ zu approximieren.
